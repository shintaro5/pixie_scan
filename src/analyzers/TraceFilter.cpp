...
void TraceFilter::CalcBaseline(void) {
    int offset = trigs_.front() - t_.GetRisetime() - 5;
...
unsigned int TraceFilter::CalcFilters(const Trace *sig) {
    try {
        ...
        CalcEnergyFilterLimits(trigs_.front());
        CalcBaseline();
...
void TraceFilter::CalcEnergyFilter(void) {
    ...
    for (unsigned int i = limits_[1]; i < limits_[2]; i++)
...
void TraceFilter::CalcEnergyFilterLimits(const unsigned int &tpos) {
    ...
    if (tpos + l + g > sig_->size())
        throw (LATE_TRIG);

    if (isVerbose_)
        cout << "********** CalcEnergyFilterLimits **********" << endl
             << "The limits for the Energy filter sums: " << endl
             << "  Rise Sum : Low = " << p0 << " High = " << p1 - 1 << endl
             << "  Gap Sum  : Low = " << p1 << " High = " << p2 - 1 << endl
             << "  Fall Sum : Low = " << p2 << " High = " << p3 << endl << endl;

    limits_.push_back(p0);      // Start of E0 sum
    limits_.push_back(p1);      // End of E0 sum / Start of gap sum
    limits_.push_back(p2);      // End of gap sum / Start of E1 sum
    limits_.push_back(p3);      // End of E1 sum
}
