...
TraceFilterAnalyzer::TraceFilterAnalyzer(const bool &analyzePileup/*=false*/, const bool &verbose/*=false*/) :
        TraceAnalyzer() {
...
void TraceFilterAnalyzer::Analyze(Trace &trace, const std::string &type,
    ...
    TraceFilter filter(globs->filterClockInSeconds() * 1e9,
                       pars.first, pars.second, analyzePileup_, verbose_);
    ...
    for (unsigned int i = 0; i < energies.size(); i++) {
        ss << "filterEnergy" << i;
        trace.SetValue(ss.str(), energies[i]);
        ss.str("");
    }
